...
(function Bookmark() {
	const { CosmosAsync, Player, LocalStorage, ContextMenu, URI } = Spotifier;
	if (!(CosmosAsync && URI)) {
	...
	// Local Storage keys
	const STORAGE_KEY = "bookmark_spotifier";
...
`;
			...
			Spotifier.Tippy(this.querySelectorAll("[data-tippy-content]"), Spotifier.TippyProps);
			if (isPlayable) {
	...
	async function storeThisPage() {
		...
		const context = Spotifier.Platform.History.location.pathname;
		try {
			contextUri = Spotifier.URI.fromString(context);
		} catch (e) {
			Spotifier.showNotification("Cannot bookmark this page", true);
			return;
		}
	...
	function getTrackMeta() {
		...
		const playerState = Spotifier.Player.data;
	...
	function createMenu() {
		...
		container.id = "bookmark-spotifier";
...
`;
	...
	async function onLinkClick(info) {
		if (info.context?.startsWith("/")) {
			Spotifier.Platform.History.push(info.context);
			return;
		}
		const url = Spotifier.URI.fromString(info.uri).toURLPath(true);
		Spotifier.Platform.History.push(url);
	}
	...
	function onPlayClick(info) {
		...
		Spotifier.Player.playUri(uri, {}, options);
	}

	const fetchAlbum = async (uri) => {
		const { getAlbum } = Spotifier.GraphQL.Definitions;
		const { data } = await Spotifier.GraphQL.Request(getAlbum, {
			uri,
	...
	const fetchArtist = async (uri) => {
		...
		const { data } = await Spotifier.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Spotifier.Locale.getLocale(),
	...
	const fetchTrack = async (uri, uid, context) => {
		...
		let newContext;
		if (context && uid && Spotifier.URI.isPlaylistV1OrV2(context)) {
			newContext = `${Spotifier.URI.fromString(context).toURLPath(true)}?uid=${uid}`;
		}
		return {
			uri,
			title: res.name,
	...
	const fetchPlaylist = async (uri) => {
		const res = await Spotifier.CosmosAsync.get(`sp://core-playlist/v1/playlist/${uri}/metadata`, {
		...
		async ([uri], [uid] = [], context = undefined) => {
			...
			switch (type) {
				...
				case Spotifier.URI.Type.ALBUM:
					...
					break;
				case Spotifier.URI.Type.ARTIST:
					meta = await fetchArtist(uri);
					break;
				case Spotifier.URI.Type.SHOW:
					meta = await fetchShow(uri);
					break;
				case Spotifier.URI.Type.EPISODE:
					meta = await fetchEpisode(uri);
					break;
				case Spotifier.URI.Type.PLAYLIST:
				case Spotifier.URI.Type.PLAYLIST_V2:

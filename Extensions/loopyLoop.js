...
`;
	...
	function saveState() {
		...
		if (!uri) return;
		Spotifier.LocalStorage.set(`loopyLoop:${uri}`, JSON.stringify({ start, end, skipZones }));
	}
	...
	function loadState() {
		...
		if (!uri) return;
		try {
			const saved = Spotifier.LocalStorage.get(`loopyLoop:${uri}`);
			if (saved) {
	...
	function applyMoveAdjustment(deltaSeconds) {
		const durationMs = Spotifier.Player.getDuration();
	...
	Spotifier.Player.addEventListener("onprogress", (event) => {
		...
		const durationMs = Spotifier.Player.getDuration() || 0;
		...
		if (prevProgressPercent > threeSecFrac && percent < nearZeroFrac) {
			if (prevPressedAt > 0 && ts - prevPressedAt < 1500) {
				...
				prevPressedAt = ts;
				prevProgressPercent = percent;
				if (start !== null) Spotifier.Player.seek(start);
				return;
			}
		}
		prevProgressPercent = percent;
		...
		if (end !== null && percent >= end) {
			// Spotifier.Player.getRepeat(): 0 = off, 1 = repeat context, 2 = repeat track
			if (Spotifier.Player.getRepeat() === 2) {
				if (ts - lastEndLoopSeek > 500) {
					lastEndLoopSeek = ts;
					Spotifier.Player.seek(start ?? 0);
				}
			} else if (ts - lastNextCall > 2000) {
				lastNextCall = ts;
				seekStartPendingUri = Spotifier.Player.data?.item?.uri ?? null;
			...
			for (let i = 0; i < skipZones.length; i++) {
				...
				if (percent >= zone.start && percent < zone.end) {
					...
					if (i !== lastSkippedZoneIdx || ts - lastSkipSeek > 500) {
						...
						Spotifier.Player.seek(zone.end);
					}
					break;
				}
			}
	...
	Spotifier.Player.addEventListener("songchange", () => {
		...
		if (Spotifier.Player.data?.item?.uri !== seekStartPendingUri) seekStartPendingUri = null;
		loadState();
		drawOnBar();
		drawSkipMarkers();
	...
	const startBtn = createMenuItem("Set song start", () => {
		if (end !== null && mouseOnBarPercent >= end) {
			Spotifier.showNotification("Song start must be before song end");
			return;
		}
	...
	const endBtn = createMenuItem("Set song end", () => {
		if (start !== null && mouseOnBarPercent <= start) {
			Spotifier.showNotification("Song end must be after song start");
			return;
		}
	...
	const skipEndBtn = createMenuItem("Set section skip end", () => {
		if (pendingSkipStart === null) {
			Spotifier.showNotification("No section skip start selected!");
			return;
		}
		...
		if (e > s) {
			if (skipZones.length < 10) {
				...
				Spotifier.showNotification("Maximum 10 skip zones reached");
			}
		}
		pendingSkipStart = null;
	});
	...
	function tryLoadInitialState(attemptsLeft) {
		if (Spotifier.Player.data?.item?.uri) {
			loadState();
			drawOnBar();
			drawSkipMarkers();
	...
	try {
		...
		const toolbarBtn = new Spotifier.Playbar.Button("Loopy Loop", markerIcon, () => {
			mouseOnBarPercent = Spotifier.Player.getProgressPercent();

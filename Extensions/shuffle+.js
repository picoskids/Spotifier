...
(async function shufflePlus() {
	if (!(Spotifier.CosmosAsync && Spotifier.Platform)) {
		setTimeout(shufflePlus, 300);
		return;
	}

	const { React } = Spotifier;
	const { useState } = React;
	let playbarButton = null;

	function getConfig() {
		try {
			const parsed = JSON.parse(Spotifier.LocalStorage.get("shufflePlus:settings"));
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Spotifier.LocalStorage.set("shufflePlus:settings", "{}");
			return {
				artistMode: "all",
	...
	function saveConfig() {
		Spotifier.LocalStorage.set("shufflePlus:settings", JSON.stringify(CONFIG));
	}
	...
	new Spotifier.Menu.Item("Shuffle+", false, settingsPage, "shuffle").register();

	const { Type } = Spotifier.URI;

	function shouldAddShufflePlus(uri) {
		if (uri.length === 1) {
			const uriObj = Spotifier.URI.fromString(uri[0]);
			switch (uriObj.type) {
				case Type.PLAYLIST:
	...
	function shouldAddShufflePlusLiked(uri) {
		const uriObj = Spotifier.URI.fromString(uri[0]);
		if (Spotifier.Platform.History.location.pathname === "/collection/tracks") {
			switch (uriObj.type) {
				case Type.TRACK:
					return true;
			}
	...
	function shouldAddShufflePlusLocal(uri) {
		const uriObj = Spotifier.URI.fromString(uri[0]);
		if (Spotifier.Platform.History.location.pathname === "/collection/local-files") {
			switch (uriObj.type) {
				case Type.TRACK:
	...
	new Spotifier.ContextMenu.Item(
		"Play with Shuffle+",
		async (uri) => {
			if (uri.length === 1) {
	...
	function renderQueuePlaybarButton() {
		if (!playbarButton) {
			playbarButton = new Spotifier.Playbar.Button(
	...
	async function fetchPlaylistTracks(uri) {
		const res = await Spotifier.Platform.PlaylistAPI.getContents(`spotify:playlist:${uri}`, {
			limit: 9999999,
		});
	...
	async function fetchFolderTracks(uri) {
		const res = await Spotifier.Platform.RootlistAPI.getContents();
		...
		async function fetchNested(folder) {
			...
			for (const i of folder.items) {
				if (i.type === "playlist") {
					const uriObj = Spotifier.URI.fromString(i.uri);
	...
	async function fetchAlbumTracks(uri, includeMetadata = false) {
		const { queryAlbumTracks } = Spotifier.GraphQL.Definitions;
		const { data, errors } = await Spotifier.GraphQL.Request(queryAlbumTracks, {
			uri,
			offset: 0,
			limit: 100,
		});
	...
	async function scanForTracksFromAlbums(res, artistName, type) {
		...
		for (const album of res) {
			...
			Spotifier.showNotification(`${artistFetchTypeCount[type]} / ${res.length} ${type}s`);
	...
	async function fetchArtistTracks(uri) {
		...
		const discography = await Spotifier.GraphQL.Request(queryArtistDiscographyAll, {
			uri,
			offset: 0,
		...
		const overview = await Spotifier.GraphQL.Request(queryArtistOverview, {
			uri,
	...
	async function fetchArtistLikedTracks(uri) {
		const artistRes = await Spotifier.CosmosAsync.get(`sp://core-collection/unstable/@/list/tracks/artist/${uri}?responseFormat=protobufJson`);
	...
	async function fetchArtistTopTenTracks(uri) {
		const { queryArtistOverview } = Spotifier.GraphQL.Definitions;
		const { data, errors } = await Spotifier.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Spotifier.Locale.getLocale(),
			includePrerelease: false,
		});
	...
	async function fetchLikedTracks() {
		const limit = 9999999;
		const res = await Spotifier.Platform.LibraryAPI.getTracks({ limit });

		return res.items.filter((track) => track.isPlayable).map((track) => track.uri);
	}

	async function fetchLocalTracks() {
		const res = await Spotifier.Platform.LocalFilesAPI.getTracks();

		return res.map((track) => track.uri);
	}

	function fetchQueue() {
		const { _queueState } = Spotifier.Platform.PlayerAPI._queue;
	...
	async function fetchCollection(uriObj) {
		...
		const { pathname } = Spotifier.Platform.History.location;
	...
	async function fetchShows(uri) {
		const res = await Spotifier.CosmosAsync.get(`sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`);
	...
	async function Queue(list, context, type) {
		...
		const { _queue, _client } = Spotifier.Platform.PlayerAPI._queue;
		...
		if (context) {
			const { sessionId } = Spotifier.Platform.PlayerAPI.getState();
			Spotifier.Platform.PlayerAPI.updateContext(sessionId, {
				uri: context,
				url: `context://${context}`,
			});
		}
		...
		switch (type) {
			case Type.ARTIST:
				if (CONFIG.artistMode === "topTen") {
					Spotifier.showNotification(`Shuffled Top ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "likedSongArtist") {
					Spotifier.showNotification(`Shuffled ${count} Liked Songs`);
					break;
				}
				if (CONFIG.artistMode === "single") {
					Spotifier.showNotification(`Shuffled ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "album") {
					Spotifier.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, Total of ${count} Songs`);
					break;
				}
				Spotifier.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
				break;
			default:
				Spotifier.showNotification(`Shuffled ${count} Songs`);
		}
	...
	async function fetchAndPlay(rawUri) {
		...
		try {
			if (rawUri === "queue") {
				...
				const uriObj = Spotifier.URI.fromString(rawUri);
				type = uriObj.type;
				...
				if (!list?.length) {
					Spotifier.showNotification("Nothing to play", true);
					return;
				}
			...
			Spotifier.showNotification(String(error), true);
			console.error(error);
		}
	}
})();

...
let dismissed;
try {
	dismissed = JSON.parse(Spotifier.LocalStorage.get("new-releases:dismissed"));
...
class Grid extends react.Component {
	...
	removeCards(id, type) {
		switch (type) {
			case "reset":
				Spotifier.showNotification("Reset dismissed releases");
				dismissed = [];
				break;
			case "undo":
				if (!dismissed[0]) Spotifier.showNotification("Nothing to undo", true);
				else Spotifier.showNotification("Undone dismissal");
		...
		Spotifier.LocalStorage.set("new-releases:dismissed", JSON.stringify(dismissed));
	...
	async componentDidMount() {
		...
		this.configButton = new Spotifier.Menu.Item(
			"New Releases config",
			false,
			openConfig,
	...
	render() {
		...
		const version = Spotifier.Platform.version.split(".").map((i) => Number.parseInt(i));
		...
		return react.createElement(
			...
			react.createElement(
				...
				react.createElement("h1", null, Spotifier.Locale.get("new_releases")),
				react.createElement(
					"div",
					{
					...
					react.createElement(ButtonText, {
						text: Spotifier.Locale.get("playlist.extender.refresh"),
...
async function getArtistList() {
	...
	const artists = await Spotifier.Platform.LibraryAPI.getContents(config);
	count(true);
	return artists.items ?? [];
}

async function getArtistEverything(artist) {
	const { data, errors } = await Spotifier.GraphQL.Request(
		{
	...
	const items = [];
	const types = [
		[CONFIG.album, releases.filter((r) => r.type === "ALBUM"), Spotifier.Locale.get("album")],
		...
		// [CONFIG["appears-on"], releases.appears_on?.releases, Spotifier.Locale.get("artist.appears-on")],
		[CONFIG.compilations, releases.filter((r) => r.type === "COMPILATION"), Spotifier.Locale.get("compilation")],
		[
			CONFIG["single-ep"],
			releases.filter((r) => r.type === "SINGLE" || r.type === "EP"),
			`${Spotifier.Locale.get("single")}/${Spotifier.Locale.get("ep")}`,
		],
	];
...
async function getPodcastList() {
	const body = await Spotifier.Platform.LibraryAPI.getShows({ limit: 50000 });
	return body.items ?? [];
}

async function getPodcastRelease(uri) {
	const body = await Spotifier.Platform.ShowAPI.getContents(uri, { limit: 50000 });
	return body.items;
}
...
async function fetchTracks() {
	...
	Spotifier.showNotification(`Fetching releases from ${artistList.length} artists`);
...
async function fetchPodcasts() {
	const items = [];
	const itemTypeStr = Spotifier.Locale.get("card.tag.episode");

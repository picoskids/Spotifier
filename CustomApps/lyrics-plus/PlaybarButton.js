(function PlaybarButton() {
	if (!Spotifier.Platform.History) {
		setTimeout(PlaybarButton, 300);
		return;
	}

	const button = new Spotifier.Playbar.Button(
		"Lyrics Plus",
		...
		() =>
			Spotifier.Platform.History.location.pathname !== "/lyrics-plus"
				? Spotifier.Platform.History.push("/lyrics-plus")
				: Spotifier.Platform.History.goBack(),
		false,
		Spotifier.Platform.History.location.pathname === "/lyrics-plus",
		false
	);
	...
	if (Spotifier.LocalStorage.get("lyrics-plus:visual:playbar-button") === "true") setPlaybarButton();
	...
	Spotifier.Platform.History.listen((location) => {

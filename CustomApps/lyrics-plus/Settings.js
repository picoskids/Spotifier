...
const RefreshTokenButton = ({ setTokenCallback }) => {
	...
	useEffect(() => {
		if (buttonText === "Refreshing token...") {
			Spotifier.CosmosAsync.get("https://apic-appmobile.musixmatch.com/ws/1.1/token.get?app_id=mac-ios-v2.0", null, {
...
const ConfigAdjust = ({ name, defaultValue, step, min, max, onChange = () => {} }) => {
	...
	return react.createElement(
		...
		react.createElement(
			...
			react.createElement(SwapButton, {
				icon: Spotifier.SVGIcons.plus2px,
				onClick: () => adjust(1),
				disabled: value === max,
			})
		)
...
const ConfigHotkey = ({ name, defaultValue, onChange = () => {} }) => {
	const [value, setValue] = useState(defaultValue);
	const [trap] = useState(new Spotifier.Mousetrap());
	...
	return react.createElement(
		...
		react.createElement(
			...
			react.createElement(
				...
				react.createElement(SwapButton, {
					icon: Spotifier.SVGIcons["chart-up"],
					onClick: () => onSwap(item.name, -1),
					disabled: isFirst,
				}),
				react.createElement(SwapButton, {
					icon: Spotifier.SVGIcons["chart-down"],
					onClick: () => onSwap(item.name, 1),
					disabled: isLast,
				}),
				react.createElement(ButtonSVG, {
...
const corsProxyTemplate = () => {
	const [proxyValue, setProxyValue] = react.useState(localStorage.getItem("spotifier:corsProxyTemplate") || "https://cors-proxy.spotifier.app/{url}");
	...
	return react.createElement("input", {
		...
		onChange: (event) => {
			...
			if (value === "" || !value) return localStorage.removeItem("spotifier:corsProxyTemplate");
			localStorage.setItem("spotifier:corsProxyTemplate", value);
		},
	});
};
...
function openConfig() {
	const configContainer = react.createElement(
		...
		react.createElement("span", {
			dangerouslySetInnerHTML: {
				__html: "Spotify will reload its webview after applying. Leave empty to restore default: <code>https://cors-proxy.spotifier.app/{url}</code>",
			},
		})
	);

const ProviderGenius = (() => {
	...
	async function getNote(id) {
		const body = await Spotifier.CosmosAsync.get(`https://genius.com/api/annotations/${id}`);
		const response = body.response;
		let note = "";

		// Authors annotations
		if (response.referent && response.referent.classification === "verified") {
			const referentsBody = await Spotifier.CosmosAsync.get(`https://genius.com/api/referents/${id}`);
	...
	async function fetchLyrics(info) {
		...
		for (const title of titles) {
			...
			const geniusSearch = await Spotifier.CosmosAsync.get(url);

const Utils = {
	addQueueListener(callback) {
		Spotifier.Player.origin._events.addListener("queue_update", callback);
	},
	removeQueueListener(callback) {
		Spotifier.Player.origin._events.removeListener("queue_update", callback);
	},
			...
			if (line.trim() !== "") {
				if (isKaraoke) {
					if (!lyricContent.endsWith(">")) {
						...
						const endTime = lines[i + 1]?.match(syncedTimestamp)?.[1] || this.formatTime(Number(Spotifier.Player.data.item.metadata.duration));

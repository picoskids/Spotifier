...
void (async function checkForUpdate() {
  ...
  const { check_spotifier_update, version } = Spotifier.Config;
  // Skip checking if upgrade check is disabled, or version is Dev/version is not set
  if (!check_spotifier_update || !version || version === "Dev") return;
  // Fetch latest version from GitHub
  try {
    let changelog;
    const res = await fetch("https://api.github.com/repos/spotifier/cli/releases/latest");
    ...
    if (semver !== version) {
      ...
      content.id = "spotifier-update";
      content.innerHTML = `
				<style>
					#spotifier-update a {
						text-decoration: underline;
					}
					#spotifier-update pre {
						cursor: pointer;
						font-size: 1rem;
						padding: 0.5rem;
					...
					#spotifier-update ol > li {
						list-style-type: decimal;
					}
					.spotifier-update-space {
						margin-bottom: 25px;
					}
					.spotifier-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="spotifier-update-space">Update Spotifier to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update Spotifier CLI</li>
					<pre class="spotifier-update-little-space">spotifier update</pre>
					<p>Spotifier will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed Spotifier via a package manager, update using said package manager.</p>
				</ol>
			`;

      void (async function attachUpdateTippy() {
        const Tippy = await waitFor(() => Spotifier.Tippy, 300);
        ...
        for (const instance of tippy) {
          instance.reference.addEventListener("click", () => {
            Spotifier.Platform.ClipboardAPI.copy(instance.reference.textContent);
      ...
      new Spotifier.Topbar.Button(
        ...
        () => Spotifier.PopupModal.display(updateModal),
      );
    }
  } catch (err) {
    console.error(err);

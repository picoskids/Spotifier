...
var (
	spotifierFolder         = utils.GetSpotifierFolder()
	rawFolder, themedFolder = getExtractFolder()
	backupFolder            = utils.GetStateFolder("Backup")
	userThemesFolder        = utils.GetSubFolder(spotifierFolder, "Themes")
	quiet                   bool
	isAppX                  = false
	spotifyPath             string
	prefsPath               string
...
// tries to auto-detect them and stops spotifier when any one
// of them is invalid.
func InitPaths() {
	...
	if isAppX {
		appDestPath = filepath.Join(spotifierFolder, "AppX")
	} else {
		appDestPath = appPath
	}
...
func GetConfigPath() string {
	return filepath.Join(spotifierFolder, "config-xpui.ini")
}
...
func CheckUpdate(version string) {
	if !settingSection.Key("check_spotifier_update").MustBool() || version == "Dev" {
		return
	}
	...
	if latestTag != version {
		...
		utils.PrintInfo(`Run "spotifier update" or use a package manager to update spotifier`)
	}
}
